...
let localUsers = JSON.parse(localStorage.getItem('storeUsers')) || [];
let localProducts = JSON.parse(localStorage.getItem('storeProducts')) || [];
let localCategories = JSON.parse(localStorage.getItem('storeCategories')) || [];
let localOrders = JSON.parse(localStorage.getItem('storeOrders')) || [];
let localCart = JSON.parse(localStorage.getItem('storeCart')) || [];
...
window.showToast = (message, type = 'success') => {
    ...
    toast.innerHTML = `<span>${message}</span> <strong style="cursor:pointer; margin-left:10px;" onclick="this.parentElement.remove()">✕</strong>`;
...
// Proper isolation of roles so customers cannot access admin pages and vice-versa
...
window.checkPasswordMatch = () => {
    ...
    if (newPass || confirmPass) {
        if (newPass === confirmPass && newPass !== '') {
            msg.innerHTML = 'Passwords match'; msg.style.color = 'var(--success)';
            saveBtn.disabled = false; saveBtn.style.opacity = '1'; saveBtn.style.cursor = 'pointer';
        } else {
            msg.innerHTML = 'Passwords do not match'; msg.style.color = 'var(--danger)';
...
window.handleUpdateProfile = (event) => {
    ...
    if (currentPass || newPass || confirmPass) {
        ...
        if (userDoc && userDoc.password !== currentPass) return window.showToast('Your current password is incorrect.', 'error');
...
window.renderProducts = () => {
    ...
    container.innerHTML = filtered.map(p => `
        <div class="card">
            <img src="${p.image || 'placeholder.jpg'}" alt="${p.name}" class="card-img" style="object-fit: cover;">
            <div class="card-body">
                <h3>${p.name}</h3><p>${p.desc}</p><div class="price">RM${p.price.toFixed(2)}</div>
                ...
                <button type="button" onclick="window.addToCart('${p.id}', '${p.name.replace(/'/g, "\\'")}', ${p.price})" class="btn" style="width:100%">Add to Cart</button>
            </div>
        </div>
    `).join('');
};
...
window.addToCart = (id, name, price, specificQty) => {
    ...
    if (existing) existing.quantity += qty; else localCart.push({ id, name, price, quantity: qty });
    window.saveCart(); window.location.href = 'cart.html';
};
...
window.handleCheckout = (event) => {
    ...
    const finalTotal = subtotal + (subtotal * 0.06);
    const currentUserEmail = localStorage.getItem('loggedInEmail') || 'customer@example.com';
    const dateStr = new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' });

    const newOrder = { id: 'ord' + Date.now(), email: currentUserEmail, date: dateStr, total: finalTotal, status: 'Processing', items: [...localCart] };
...
window.updateChart = () => {
    ...
    if (selectedMonth === 'all') {
        ...
        localOrders.forEach(o => {
            ...
            if (!isNaN(d.getTime())) rev[d.getMonth()] += o.total;
        });
    ...
    window.salesChartInstance = new Chart(ctx, {
        ...
        options: {
            ...
            responsive: true,
            scales: {
                y: { beginAtZero: true } // Ensures empty charts still render the baseline!
            }
        }
    });
};

// --- MODULAR ADMIN RENDERERS (Fixes Toast Wiping Bug) ---
window.renderAdminUsers = () => {
    ...
    const render = (list, tbody) => {
        ...
        else tbody.innerHTML = list.map(u => `<tr><td>${u.firstName} ${u.lastName}</td><td>${u.email}</td><td>
...
window.renderAdminCategories = () => {
    ...
    if (!tbody) return;
    tbody.innerHTML = localCategories.map(c => `<tr><td>${c.name}</td><td>${localProducts.filter(p => p.category === c.name).length}</td><td><button class="btn btn-danger" onclick="window.deleteCategory('${c.id}', '${c.name}')">Del</button></td></tr>`).join('');
};

window.renderAdminProducts = () => {
    const tbody = document.getElementById('admin-products-tbody');
    if (!tbody) return;
    tbody.innerHTML = localProducts.map(p => `<tr><td>${p.name}</td><td>${p.category}</td><td>RM${p.price.toFixed(2)}</td><td>${p.stock}</td><td><button class="btn btn-outline" style="padding: 0.2rem 0.5rem; margin-right: 0.5rem;" onclick="window.openEditProductModal('${p.id}')">Edit</button><button class="btn btn-danger" style="padding: 0.2rem 0.5rem;" onclick="window.deleteProduct('${p.id}')">Del</button></td></tr>`).join('');

    const catDrop = document.getElementById('add-prod-cat');
    if (catDrop) catDrop.innerHTML = localCategories.map(c => `<option value="${c.name}">${c.name}</option>`).join('');
};

window.renderAdminOrders = () => {
    const tbody = document.getElementById('admin-orders-tbody');
    if (!tbody) return;
    tbody.innerHTML = localOrders.map(o => `<tr><td>#${o.id.slice(0, 6).toUpperCase()}</td><td>${o.email}</td><td>RM${o.total.toFixed(2)}</td><td>
        <select onchange="window.updateOrderStatus('${o.id}', this.value)" style="padding: 0.2rem;"><option ${o.status === 'Processing' ? 'selected' : ''}>Processing</option><option ${o.status === 'Shipped' ? 'selected' : ''}>Shipped</option><option ${o.status === 'Delivered' ? 'selected' : ''}>Delivered</option><option ${o.status === 'Cancelled' ? 'selected' : ''}>Cancelled</option></select>
        </td><td><button class="btn btn-danger" onclick="window.deleteOrder('${o.id}')">Del</button></td></tr>`).join('');
};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Populate all admin pages safely without throwing errors on other pages
    window.renderAdminUsers();
    window.renderAdminCategories();
    window.renderAdminProducts();
    window.renderAdminOrders();
    ...
    if (currentFile === 'products.html') {
        ...
        if (filterCat) filterCat.innerHTML = '<option value="">All Categories</option>' + localCategories.map(c => `<option value="${c.name}">${c.name}</option>`).join('');
        window.renderProducts();
    }
    ...
    if (currentFile === 'product-detail.html') {
        ...
        if (p) {
            ...
            document.getElementById('detail-desc').innerHTML = `<p>${p.desc}</p>`;

            // ADD THIS NEW LINE RIGHT HERE:
            if (document.getElementById('detail-image')) document.getElementById('detail-image').src = p.image || 'placeholder.jpg';

            document.getElementById('detail-qty').max = p.stock;
            document.getElementById('detail-add-btn').onclick = () => window.addToCart(p.id, p.name, p.price, parseInt(document.getElementById('detail-qty').value));
        }
    }
    ...
    if (currentFile === 'checkout.html') {
        let subtotal = 0; localCart.forEach(i => subtotal += (i.price * i.quantity));
    ...
    if (currentFile === 'dashboard.html') {
        ...
        if (tbody) {
            ...
            else tbody.innerHTML = [...custOrders].reverse().map(o => `<tr><td>#${o.id.slice(0, 6).toUpperCase()}</td><td>${o.date}</td><td>RM${o.total.toFixed(2)}</td><td>${o.status}</td></tr>`).join('');
        }
    }
    ...
    if (currentFile === 'admin-dashboard.html') {
        ...
        localOrders.forEach(o => { if (o.date === todayStr) rev += o.total; if (o.status !== 'Delivered' && o.status !== 'Cancelled') count++; });
...
// Admin Global Functions (Updated to instantly re-render without reloading the browser!)
...
window.deleteCategory = (id, name) => {
    ...
    localProducts.forEach(p => { if (p.category === name) p.category = 'General' });
    localStorage.setItem('storeProducts', JSON.stringify(localProducts));
...
window.openEditProductModal = (id) => {
    ...
    document.getElementById('edit-prod-cat').innerHTML = localCategories.map(c => `<option value="${c.name}" ${c.name === p.category ? 'selected' : ''}>${c.name}</option>`).join('');
...
window.saveProductEdit = (event) => {
    ...
    if (prod) {
        ...
        prod.price = parseFloat(document.getElementById('edit-prod-price').value);
        prod.stock = parseInt(document.getElementById('edit-prod-stock').value);
        prod.category = document.getElementById('edit-prod-cat').value;
        prod.desc = document.getElementById('edit-prod-desc').value;
        localStorage.setItem('storeProducts', JSON.stringify(localProducts));
...
window.adminAddProduct = (e) => {
    ...
    localProducts.push({
        ...
        desc: document.getElementById('add-prod-desc').value
    });
    localStorage.setItem('storeProducts', JSON.stringify(localProducts));
